...
class UnifiedDashboard:
    ...
    def _initialize_state(self):
        defaults = {
            'page': 'main', 'latest_alerts': [],
            'current_status': {"message": "데이터 수신 대기 중...", "timestamp": "N/A"},
            'sound_enabled': False, 'live_df': pd.DataFrame(),
            'last_sensor_values': {"CH4": 0.0, "EtOH": 0.0, "H2": 0.0, "NH3": 0.0, "CO": 0.0},
            'sound_primed': False, 'play_sound_trigger': None,
        }
    ...
    def _process_queues(self):
        ...
        while not self.alerts_queue.empty():
            ...
            # ⭐️ 어떤 메시지든 수신하면 '시스템 현재 상태'를 업데이트
            st.session_state.current_status = {
                "message": msg.get("message", "새로운 경보 수신됨"),
            ...
            if alert_type in ["fire", "safety"]:
                ...
                # ⭐️ 경보 메시지를 DB에 저장할 때도 KST로 통일
                try:
                    # ISO 형식 문자열을 KST datetime 객체로 변환
                    msg['timestamp'] = datetime.fromisoformat(msg['timestamp'].replace('Z', '+00:00')).astimezone(KST)
                except (ValueError, TypeError, KeyError):
                    # 실패 시 현재 KST 시간으로 저장
                    msg['timestamp'] = now_kst
        ...
        while not self.sensors_queue.empty():
            ...
            try:
                ...
                # ⭐️ 센서 데이터를 저장할 때부터 KST로 저장
                data_dict['timestamp'] = datetime.now(KST)
    ...
    def _check_and_trigger_sensor_alerts(self, data_dict):
        def log_alert(message):
            try:
                # ⭐️ 로그 파일에도 KST로 시간 기록
                with open(LOG_FILE, "a", encoding="utf-8") as log_file:
                    log_file.write(f"{datetime.now(KST).isoformat()} - {message}\n")
            except Exception as e:
    ...
    def _render_header_and_nav(self):
        ...
        pages = {
            'main': '🏠 안전 모니터링', 'sensor_dashboard': '📈 실시간 센서',
            'sensor_log': '📜 센서 로그', 'crack_monitor': '🛣️ 도로 균열 감지',
    ...
    def _render_sidebar(self):
        with st.sidebar:
            ...
            if st.session_state.page in ['crack_monitor', 'hivis_monitor']:
                if st.button("새로고침 🔄"):
                    st.rerun()
    ...
    def _render_main_page(self):
        ...
        if not st.session_state.latest_alerts and self.collections:
            try:
                ...
                alerts = list(self.collections['alerts'].find(query).sort("timestamp", pymongo.DESCENDING).limit(10))
        ...
        if not st.session_state.latest_alerts:
            ...
            # ⭐️ DB에 저장된 KST 시간을 변환 없이 바로 문자열로 포맷팅
            df['timestamp'] = pd.to_datetime(df['timestamp']).dt.strftime('%Y-%m-%d %H:%M:%S')
            display_df = df.rename(columns={"timestamp": "발생 시각", "type": "유형", "message": "메시지"})
            st.dataframe(display_df[['발생 시각', '유형', '메시지']], use_container_width=True, hide_index=True)
    ...
    def _render_sensor_dashboard(self):
        ...
        status_cols[0].metric("현재 시간 (KST)", datetime.now(KST).strftime("%H:%M:%S"))

        if not df.empty and 'timestamp' in df.columns:
            # ⭐️ DB에 저장된 KST 시간을 변환 없이 바로 사용
            last_reception = pd.to_datetime(df['timestamp'].iloc[-1])
            time_diff = datetime.now(KST) - last_reception.tz_localize(KST)
            status_cols[1].metric("마지막 수신 (KST)", last_reception.strftime("%H:%M:%S"))
        ...
        if not df.empty:
            ...
            if not any([flame_detected, oxygen_unsafe, no2_dangerous, no2_warning]):
                ...
                if log_lines:
                    ...
                    for line in reversed(log_lines):
                        if " - " in line:
                            parts = line.split(" - ", 1)
                            try:
                                # ⭐️ 로그에 KST로 저장된 시간을 변환 없이 바로 표시
                                dt = datetime.fromisoformat(parts[0])
                                log_entries.append({
                                    "감지 시간 (KST)": dt.strftime('%Y-%m-%d %H:%M:%S'),
                                    "메시지": parts[1].strip()
                                })
                            except ValueError:
                                log_entries.append({"감지 시간 (KST)": "N/A", "메시지": parts[1].strip()})
                    log_df = pd.DataFrame(log_entries)
                    st.dataframe(log_df, use_container_width=True, hide_index=True)
                    csv_data = log_df.to_csv(index=False).encode('utf-8-sig')
                    st.download_button("📥 로그 CSV 다운로드", csv_data, f"sensor_log_{datetime.now(KST).strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
                    st.divider()
    ...
    def _render_crack_monitor_page(self):
        ...
        if self.collections and 'crack' in self.collections:
            try:
                for doc in self.collections['crack'].find({}).sort("timestamp", -1).limit(limit):
                    # ⭐️ DB에 저장된 KST 시간을 변환 없이 바로 포맷팅
            ...
            st.warning("DB 연결 실패로 도로 균열 데이터를 표시할 수 없습니다.")
    ...
    def _render_hivis_monitor_page(self):
        ...
        if self.collections and 'hivis' in self.collections:
            try:
                for doc in self.collections['hivis'].find({}).sort("timestamp", -1).limit(limit):
                    # ⭐️ DB에 저장된 KST 시간을 변환 없이 바로 포맷팅
                    timestamp_local = doc['timestamp'].strftime('%Y-%m-%d %H:%M:%S')
                    device_name = doc.get('source_device', 'N/A')
                    num_detections = len(doc.get('detections', []))
                        ...
                        with col2:
                            ...
                            if not detections:
                                ...
                                for i, detection in enumerate(detections):
                                    st.metric(label=f"#{i+1}: {detection['class_name']}", value=f"{detection['confidence']:.2%}")
                                    st.code(f"Box: {[int(c) for c in detection['box_xyxy']]}", language="text")
                            st.caption(f"DB ID: {doc['_id']}")
            except Exception as e:
                st.error(f"안전 조끼 데이터 로딩 중 오류 발생: {e}")
        else:
            st.warning("DB 연결 실패로 안전 조끼 데이터를 표시할 수 없습니다.")

    def _handle_audio_playback(self):
        st.html("""
            <audio id="fire-alert-sound" preload="auto"><source src="app/static/fire_cut_mp3.mp3" type="audio/mpeg"></audio>
            <audio id="safety-alert-sound" preload="auto"><source src="app/static/Stranger_cut_mp3.mp3" type="audio/mpeg"></audio>
        """)
    ...
    def run(self):
        ...
        # st_autorefresh(interval=3000, key="refresher") # 잦은 에러 발생 시 주석 처리하거나 간격을 늘리세요.
